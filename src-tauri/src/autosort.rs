...
pub fn autosort_folders_and_gen_config(
    ...
    let tags = tags
        .iter()
        .map(|x| serde_json::from_str::<AutosortTag>(x).unwrap())
        .collect::<Vec<AutosortTag>>();

    dbg!(&tags);

    let config = ProjectConfig {
        project_root: root.clone().replace("\\", "/"),
    ...
    for tag in tags {
        fs::create_dir_all(format!("{}/{}/", &root, &tag.folder))
            .expect("Cannot create the folder");
    ...
    let reader =
        fs_extra::dir::get_dir_content(&root).expect("Unable to read the directory contents");

    fs::write(
        format!("{}/project.frok.yaml", &root),
        serde_yml::to_string(&config).unwrap(),
    )
    .expect("Unable to write the config");
    ...
    for path in reader.files {
        ...
        if &file_name == "project.frok.yaml" || &file_name == "project.frok.yml" {
            continue;
        }
